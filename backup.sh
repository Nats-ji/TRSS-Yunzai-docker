...
TOTAL_SIZE=`du -sk --apparent-size --total lagrange/ redis/ yunzai/ | cut -f 1 | tail -n 1`
CHECKPOINT=`echo ${TOTAL_SIZE}/50 | bc`
echo "Creating backup file. Total size: $(numfmt --to=iec $(echo ${TOTAL_SIZE}*1000 | bc))."
echo "Estimated: [==================================================]"
echo -n "Progess:   ["
tar -c --record-size=1K --checkpoint="${CHECKPOINT}" --checkpoint-action="ttyout=>" -f - lagrange/ redis/ yunzai/ | gzip > $OUTPUT_NAME
echo "]"
echo "Done"
